...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('mosaics.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('mosaics.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('mosaics.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
